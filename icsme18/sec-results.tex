...
\centering
% \hspace*{-0.7cm}
\includegraphics[width=1\linewidth]{phase1-survey-answers3-en.png}
...
\toprule
\textsc{Brain UI Component}$^{(60)}$            & Componentes de UI com lógicas de negócio.  \\
\textsc{Coupled UI Component}$^{(18)}$           & Componentes de UI com referência concretas um para o outro.  \\
\textsc{Suspicious Behavior}$^{(17)}$              & \textit{Listener} sendo implementado dentro de outro componente de UI.  \\
\textsc{Fool Adapter}$^{(13)}$                  & Adapters que não usam o padrão \textit{ViewHolder}.  \\
\textsc{Excessive Use Of Fragments}$^{(9)}$           & Uso de \textit{fragments} sem uma necessidade explícita. \\
\textsc{UI Component Doing IO}$^{(9)}$          & Componentes de UI fazendo acesso a internet ou banco de dados.  \\
\textsc{No Fragment Usage}$^{(8)}$                  & Não usar nenhum \textit{Fragment}.  \\
\textsc{Absence of Architecture}$^{(6)}$              & Aplicativos sem uma arquitetura conhecida.  \\
\textsc{Flex Adapter}$^{(6)}$                     & \textit{Adapters} com condicionais e \textit{loops}. \\
\textsc{Resource Name Non Patterned}$^{(24)}$      & Recursos com nomes despadronizados.      \\
\textsc{Magic Resource}$^{(23)}$                      & Textos, números ou cores ``\textit{hardcoded}''.   \\
\textsc{Deep Nested Layout}$^{(19)}$       & Recurso de layout com mais de três níveis de \textit{Viwes} aninhadas.   \\
\textsc{Avoid Tradicional Images}$^{(18)}$      & Imagens que poderiam ser transformadas em recurso gráfico.   \\
\textsc{Layout Long or Repeated}$^{(14)}$            & Recurso de \textit{layout} muito longo ou com trechos de código similares ou repetidos.   \\
\textsc{Missing Image}$^{(12)}$                     & Imagem sem todas as resoluções padrões.   \\
\textsc{God Style Resource}$^{(8)}$              & Recurso de estilo único e longo.   \\
\textsc{Messy String Resource}$^{(8)}$          & Recursos de \textit{string} sem um padrão de nomenclatura.   \\
\textsc{Repeated Style Attributes}$^{(7)}$        & Repeated Style Attributes em recursos de \textit{layout} ou \textit{style}.   \\
\textsc{Inappropriate String Reuse}$^{(6)}$           & \textit{Strings} sendo reutilizadas indevidamente.    \\
\textsc{Hided Listener}$^{(5)}$                   & Atributo \textit{onClick} em recursos de \textit{layout}.  \\
\bottomrule
\end{tabular}
\label{tab:Smells}
\end{table*}
...
\begin{subfigure}[t]{.48\textwidth}\centering
  ...
  \caption{AC=Flex Adapter, CA=Coupled UI Component, CS=Suspicious Behavior, CC=Brain UI Component, LE=God Style Resource, LA=Deep Nested Layout
