...
Há quem acredite que o livro de Webster (1995) \cite{Webster:95} foi o primeiro livro a usar o termo ``mau cheiro de código''. Neste livro são apresentadas diversas más práticas em Orientação a Objetos. Longos métodos e complexidade excessiva são dois exemplos. 

Entretanto, o termo se popularizou no livro de Fowler (1999) \cite{Refactoring:99}. No livro ele diz que \textit{``um mau cheiro de código são indicações de que existe um problema que pode ser resolvido através de refatorações''}. Refatoração, segundo Fowler, é o ato de alterarmos o código sem alterar seu comportamento com o objetivo de melhorar a qualidade do código em termos de legibilidade e manutenibilidade. Essas indicações provém do conhecimento empírico de desenvolvedores experientes, que ao longo dos anos, após trabalharem em diversos códigos, são capazes de listar características que podem indicar que um trecho de código é problemático. O livro apresenta 22 maus cheiros.

Exemplo de mau cheiro de Fowler.
...
Clean Code, e outros livros

\subsection{Como são documentados maus cheiros? Livros e acadêmia}
