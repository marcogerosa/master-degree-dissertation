...
\subsection{Padrões de Projeto}
  ...
  Para se documentar um padrão, é comum seguir um formato específico. O formato indica onde e como cada aspecto, como problema, contexto ou solução, será encontrado. Alguns formatos incluem uma série de outras informações além detas três. Ao longo dos anos, alguns formatos foram se destacando. A Figura \ref{fig:formato-padroes} apresenta a tradução de um gráfico criado por Joshua Kerievsky \cite{PatternsFormats} contendo quatro formatos existentes de se escrever padrões. Os formatos são nomeados como Portland, Coplien, \acs{GoF} e Alexandrino, e foram posicionadas no gráfico de acordo com seu nível de maturidade e clareza.
