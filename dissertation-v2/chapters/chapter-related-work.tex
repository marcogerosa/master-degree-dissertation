...
\label{android-vs-tradicional}
  ...
  Minelli e Lanza \cite{Mantyla2013} apresentam diferenças no desenvolvimento de aplicativos e softwares tradicionais em termos de métricas de código, uso de APIs terceiras e evolução \todo{citar, Cresman??}. Para isso se utilizam da ferramenta SAMOA desenvolvida por eles de análise estática de código.
...
\label{maus-cheiros-android}
  ...
  \subsection{Presença de maus cheiros tradicionais em aplicativos Android}
    ...
    É interessante notar que dentro da definição de Verloop \cite{MobileSmells:13} de classes núcleo, estão incluídas classes que herdam de \texttt{Services} e todas as demais que herdam de alguma classe do SDK Android, porém as únicas classes que apresentaram maus cheiros foram \texttt{Activities} e \texttt{Adapters}. Como vimos em \ref{sec:Android}, essas classes são responsáveis por lidar com o \textit{front-end} Android, o que reforça nossa hipótese de que o \textit{front-end} Android tende a ser mais problemático que o restante dos códigos da aplicação e por isso vale a pena ser estudado mais a fundo. 
