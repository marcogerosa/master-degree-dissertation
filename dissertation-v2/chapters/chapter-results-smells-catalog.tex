...
\subsubsection{Maus cheiros em componentes do front-end Android}
    ...
    \caption{Maus cheiros em componentes da camada de apresentação Android e breve descrição dos sintomas.}
    \footnotesize 
    ...
    \textsc{Componente de UI Zumbi}$^{2\star}$              & Componentes com referência a componentes de UI com ciclo de vida.  \\
    \textsc{Uso Excessivo de Fragments}$^{\star}$           & Uso de \textit{fragments} sem uma necessidade explícita. \\
    \textsc{Componente de UI Fazendo IO}$^{\star}$          & Componentes de UI fazendo acesso a internet ou banco de dados.  \\
    \textsc{Não Uso de Fragment}$^{\star}$                  & Não usar nenhum \textit{Fragment}.  \\
    \textsc{Ausência de Arquitetura}$^{\star}$              & Aplicativos sem uma arquitetura conhecida.  \\
    ...
    % \noindent
    % \textsc{\textbf{{\small Componente de UI Inteligente}}}$^{12\star}$ \\
    % \textbf{\small Elementos afetados:} \textit{Activities}$^{5\star}$, \textit{Fragments}$^{3\star}$, \textit{Adapters}$^{2\star}$ e \textit{Listeners}. \\
    ...
    \noindent
    \textsc{\textbf{{\small Adapter Consumista}}}$^{2\star}$ São indícios do mau cheiro quando \textit{Adapters}$^{2\star}$ não reutilizam instâncias das \textit{views} que representam os campos a serem populados para cada item da coleção através do padrão \textit{View Holder} ou quando os mesmos possuem classes internas para reaproveitamento das \textit{views} porém não são estáticas.
    ...
    \textsc{\textbf{{\small Componente de UI Fazendo IO}}}$^{\star}$ 
        \textit{Activities}$^{\star}$, \textit{Fragments} e \textit{Adapters} não devem ser responsáveis por operações de IO. São indícios do mau cheiro implementações de acesso a banco de dados ou internet a partir desses elementos.
    ...
    \noindent
    \textsc{\textbf{{\small Não Uso de Fragment}}}$^{\star}$ \textit{Fragments}$^{\star}$ devem ser usados sempre que possível em conjunto com \textit{Activities}. É indício do mau cheiro a não existência de \textit{Fragments} na aplicação ou o uso de \textit{EditTexts}, \textit{Spinners} ou outras \textit{views} por \textit{Activities}.
    ...
    \noindent
    \textsc{\textbf{{\small Adapter Complexo}}}$^{\star}$ \textit{Adapters}$^{\star}$ devem ser responsáveis por popular uma \textit{view} a partir de um único objeto, sem realizar lógicas ou tomadas de decisão. São indícios desse mau cheiro quando \textit{Adapters} contém muitos condicionais (\textit{if} ou \textit{switch}) ou cálculos no método \textit{getView}, responsável pela construção e preenchimento da \textit{view}. 
...
\subsubsection{Maus cheiros em recursos Android}
    ...
    \centering
    \renewcommand*{\arraystretch}{1}
    \caption{Maus cheiros em recursos Android e breve descrição dos sintomas.}
    \footnotesize 
    ...
    \textsc{Imagem Tradicional Dispensável}$^{3\star}$                    & Imagens que poderiam ser transformadas em recurso gráfico.   \\
    ...
    \textsc{Atributos de Estilo Repetidos}$^{\star}$                     & Atributos de estilo repetidos em recursos de \textit{layout} ou \textit{style}.   \\
    ...
    \textbf{\textsc{{\small Nome de Recurso Despadronizado}}}$^{4\star}$
        São indícios do mau cheiro quando recursos de \textit{layout}$^{\star}$, recursos de \textit{string}$^{2\star}$, recursos de \textit{style}$^{\star}$ e recursos \textit{drawables} não possuem um padrão de nomenclatura.
    ...
    \textbf{\textsc{{\small Layout Profundamente Aninhado}}}$^{3\star}$
        São indícios desse mau cheiro o uso de profundos aninhamentos na construção de recursos de \textit{layout}$^{3\star}$, ou seja, \texttt{ViewGroups} contendo outros \texttt{ViewGroups} sucessivas vezes. O site oficial do Android conta com informações e ferramentas automatizadas para lidar com esse sintoma \cite{OptmizingViewHierarchies}. 
    ...
    \noindent
    \textbf{\textsc{{\small Imagem Tradicional Dispensável}}}$^{3\star}$
    ...
    \textbf{\textsc{{\small Layout Longo ou Repetido}}}$^{2\star}$
        São indícios do mau cheiro quando recursos de \textit{layout}$^{2\star}$ é muito grande ou possue trechos de layout muito semelhantes ou iguais a outras telas. 
    ...
    \textbf{\textsc{{\small Imagem Faltante}}}$^{2\star}$
        As imagens devem ser disponibilizadas em mais de um tamanho/resolução para que o Android possa realizar otimizações. São indícios do mau cheiro haver apenas uma versão de algum recurso \textsc{drawable}$^{2\star}$ do tipo png, jpg ou gif ou ainda, ter imagens em diretórios incorretos em termos de \texttt{dpi}.
    ...
    \textbf{\textsc{{\small Longo Recurso de Estilo}}}$^{\star}$
        É indício do mau cheiro haver apenas um recursos de \textit{style}$^{\star}$ ou conter Recursos de Estilo muito longos.
    ...
    \noindent
    \textbf{\textsc{{\small Recurso de String Bagunçado}}}$^{\star}$
        É indício do mau cheiro o uso de apenas um arquivo para todos os recursos de \textit{string}$^{\star}$ do aplicativo e a não existência de um padrão de nomenclatura e separação para os recursos de \textit{string} de uma mesma tela. 
    ...
    \noindent
    \textbf{\textsc{{\small Reúso Inadequado de String}}}$^{\star}$
        Cada tela deve ter seu conjunto de recursos de \textit{string}$^{\star}$. É indício do mau cheiro reutilizar o mesmo recurso de \textit{string} em diferentes telas do aplicativo apenas porque o texto coincide.
    ...
    \textbf{\textsc{{\small Listener Escondido}}}$^{\star}$
        recursos de \textit{layout}$^{\star}$ devem ser responsáveis apenas por apresentar informações. É indício do mau cheiro o uso de atributos diretamente em recursos de \textit{layout}, como por exemplo o atributo \textit{onClick}, para configurar o \textit{Listener} que responderá ao evento.
