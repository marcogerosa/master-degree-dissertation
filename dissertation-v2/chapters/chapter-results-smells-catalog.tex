...
Nesta seção apresentamos os 21 maus cheiros derivados sendo respectivamente 10 maus cheiros em componentes do \textit{front-end} Android e 11 em recursos Android.

\subsubsection{Maus cheiros em componentes do front-end Android}
    A Tabelas \ref{tab:Smells-Java} apresenta a lista dos maus cheiros em componentes do \textit{front-end} Android, suas respectivas estrelas indicando a confiabilidade e uma breve descrição do sintoma relacionado. Em seguida, nesta seção é apresentado a descrição completa do mau cheiro.
    ...
    \toprule
    \textsc{Componente de UI Inteligentes}$^{12\star}$                      & Componentes de UI com lógicas de negócio.  \\
    \textsc{Componente de UI Acoplado}$^{3\star}$                           & Componentes de UI com referência concretas um para o outro.  \\
    ...
    \noindent
    \textsc{\textbf{{\small Componente de UI Inteligente}}}$^{12\star}$ \textit{Activities}$^{5\star}$, \textit{Fragments}$^{3\star}$, \textit{Adapters}$^{2\star}$ e \textit{Listeners} devem conter apenas códigos responsáveis por apresentar, interagir e atualizar a UI. São indícios do mau cheiro a existência de códigos relacionados a lógica de negócio, operações de IO\footnote{Ver mau cheiro \textsc{Classes de UI Fazendo IO}.}, conversão de dados ou campos estáticos nesses elementos.
    ...
    \noindent
    \textbf{\textsc{{\small Componente de UI Acoplado}}}$^{3\star}$ \textit{Fragments}$^{2\star}$, \textit{Adapters} e \textit{Listeners} não devem ter referência direta para quem os utiliza. São indícios do mau cheiro a existência de referência direta para \textit{Activities} ou \textit{Fragments} nesses elementos.
    ...
    \noindent
    \textsc{\textbf{{\small Comportamento Suspeito}}}$^{6\star}$ \textit{Activities}$^{3\star}$, \textit{Fragments}$^{2\star}$ e \textit{Adapters}$^{1\star}$ não devem ser responsáveis pela implementação do comportamento dos eventos. São indícios do mau cheiro o uso de classes anônimas, classes internas ou polimorfismo (através de \textit{implements}) para implementar \textsc{Listeners} de modo a responder a eventos do usuário.
    ...
    \noindent
    \textsc{\textbf{{\small Adapter Consumista}}}$^{5\star}$ São indícios do mau cheiro quando \textit{Adapters}$^{5\star}$ não reutilizam instâncias das \textit{views} que representam os campos a serem populados para cada item da coleção através do padrão \textit{View Holder} ou quando os mesmos possuem classes internas para reaproveitamento das \textit{views} porém não são estáticas.
    ...
    \noindent
    \textsc{\textbf{{\small Não Uso de Fragment}}}$^{3\star}$ \textit{Fragments}$^{2\star}$ devem ser usados sempre que possível em conjunto com \textit{Activities}$^{2\star}$. É indício do mau cheiro a não existência de \textit{Fragments} na aplicação ou o uso de \textit{EditTexts}, \textit{Spinners} ou outras \textit{views} por \textit{Activities}.
    ...
    \noindent
    \textsc{\textbf{{\small Classes de UI Fazendo IO}}}$^{3\star}$ \label{subsce:classes-ui-fazendo-io}
        \textit{Activities}$^{2\star}$, \textit{Fragments}$^{1\star}$ e \textit{Adapters}$^{1\star}$ não devem ser responsáveis por operações de IO. São indícios do mau cheiro implementações de acesso a banco de dados ou internet a partir desses elementos.
    ...
    \noindent
    \textsc{\textbf{{\small Arquitetura Não Identificada}}}$^{2\star}$ São indícios do mau cheiro quando diferentes \textit{Activities}$^{2\star}$ e \textit{Fragments}$^{1\star}$ no projeto apresentam fluxos de código complexos, possivelmente são \textsc{Classe de UI Inteligente}, onde não é possível identificar uma organização padronizada entre eles que aponte para algum padrão arquitetural, como por exemplo, MVC (\textit{Model View Controller}), MVP (\textit{Model View Presenter}), MVVM (\textit{Model View ViewModel}) ou \textit{Clean Architecture}.
    ...
    \noindent
    \textsc{\textbf{{\small Adapter Complexo}}}$^{2\star}$ \textit{Adapters}$^{2\star}$ devem ser responsáveis por popular uma \textit{view} a partir de um único objeto, sem realizar lógicas ou tomadas de decisão. São indícios desse mau cheiro quando \textit{Adapters} contém muitos condicionais (\textit{if} ou \textit{switch}) ou cálculos no método \textit{getView}, responsável pela construção e preenchimento da \textit{view}. 
...
\subsubsection{Maus cheiros em recursos Android}
    ...
    \noindent
    \textbf{\textsc{{\small Recurso Mágico}}}$^{8\star}$
        Todo recurso de cor, tamanho, texto ou estilo deve ser criado em seu respectivo arquivo e então ser usado. São indícios do mau cheiro quando recursos de \textit{layout}$^{2\star}$, recursos de \textit{string}$^{5\star}$ ou recursos de \textit{style}$^{1\star}$ usam alguma dessas informações diretamente no código ao invés de fazer referência para um recurso.
    ...
    \noindent
    \textbf{\textsc{{\small Nome de Recurso Despadronizado}}}$^{8\star}$
        São indícios do mau cheiro quando recursos de \textit{layout}$^{2\star}$, recursos de \textit{string}$^{4\star}$, recursos de \textit{style}$^{2\star}$ e recursos \textit{drawables}$^{1\star}$ não possuem um padrão de nomenclatura.
    ...
    \noindent
    \textbf{\textsc{{\small Layout Profundamente Aninhado}}}$^{7\star}$
        São indícios desse mau cheiro o uso de profundos aninhamentos na construção de recursos de \textit{layout}$^{7\star}$, ou seja, \texttt{ViewGroups} contendo outros \texttt{ViewGroups} sucessivas vezes. O site oficial do Android conta com informações e ferramentas automatizadas para lidar com esse sintoma \cite{OptmizingViewHierarchies}. 
    ...
    \noindent
    \textbf{\textsc{{\small Imagem Dispensável}}}$^{6\star}$
        O Android possui diversos tipos de recursos \textit{drawables}$^{6\star}$ que podem substituir imagens tradicionais como \texttt{.png}, \texttt{.jpg} ou \texttt{.gif} a um custo menor em termos de tamanho do arquivo e sem a necessidade de haver versões de diferentes tamanhos/resoluções. São indícios do mau cheiro a existência de imagens com, por exemplo, cores sólidas, degradês ou estado de botões, que poderiam ser substituídas por recursos \textit{drawables} de outros tipos como \textit{shapes}, \textit{state lists} ou \textit{nine-patch file} ou a não existência de imagens vetoriais, que podem ser redimensionadas sem a perda de qualidade.
    ...
    \noindent
    \textbf{\textsc{{\small Layout Longo ou Repetido}}}$^{5\star}$
        São indícios do mau cheiro quando recursos de \textit{layout}$^{5\star}$ é muito grande ou possue trechos de layout muito semelhantes ou iguais a outras telas. 
    ...
    \noindent
    \textbf{\textsc{{\small Imagem Faltante}}}$^{4\star}$
        As imagens devem ser disponibilizadas em mais de um tamanho/resolução para que o Android possa realizar otimizações. São indícios do mau cheiro haver apenas uma versão de algum recurso \textsc{drawable}$^{4\star}$ do tipo png, jpg ou gif ou ainda, ter imagens em diretórios incorretos em termos de \texttt{dpi}.
    ...
    \noindent
    \textbf{\textsc{{\small Longo Recurso de Estilo}}}$^{3\star}$
        É indício do mau cheiro haver apenas um recursos de \textit{style}$^{4\star}$ ou conter Recursos de Estilo muito longos.
    ...
    \noindent
    \textbf{\textsc{{\small Recurso de String Bagunçado}}}$^{3\star}$
        É indício do mau cheiro o uso de apenas um arquivo para todos os recursos de \textit{string}$^{3\star}$ do aplicativo e a não existência de um padrão de nomenclatura e separação para os recursos de \textit{string} de uma mesma tela. 
    ...
    \noindent
    \textbf{\textsc{{\small Atributos de Estilo Repetidos}}}$^{3\star}$
        É indício do mau cheiro haver recursos de \textit{layout}$^{1\star}$ ou recursos de \textit{style}$^{2\star}$ com blocos de atributos de estilo repetidos. 
    ...
    \noindent
    \textbf{\textsc{{\small Reúso Inadequado de String}}}$^{2\star}$
        Cada tela deve ter seu conjunto de recursos de \textit{string}$^{2\star}$. É indício do mau cheiro reutilizar o mesmo recurso de \textit{string} em diferentes telas do aplicativo apenas porque o texto coincide.
    ...
    \noindent
    \textbf{\textsc{{\small Listener Escondido}}}$^{2\star}$
        recursos de \textit{layout}$^{2\star}$ devem ser responsáveis apenas por apresentar informações. É indício do mau cheiro o uso de atributos diretamente em recursos de \textit{layout}, como por exemplo o atributo \textit{onClick}, para configurar o \textit{Listener} que responderá ao evento.
