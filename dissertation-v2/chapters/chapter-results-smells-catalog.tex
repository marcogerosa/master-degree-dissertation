...
\subsubsection{Maus cheiros em componentes do front-end Android}
    ...
    \toprule
    \textsc{Componente de UI Inteligente}$^{12\star}$                      & Componentes de UI com lógicas de negócio.  \\
    ...
    \noindent
    \textsc{\textbf{{\small Comportamento Suspeito}}}$^{3\star}$ \textit{Activities}, \textit{Fragments} e \textit{Adapters} não devem ser responsáveis pela implementação do comportamento dos eventos. São indícios do mau cheiro o uso de classes anônimas, classes internas ou polimorfismo (através de \textit{implements}) para implementar \textsc{Listeners}$^{2\star}$ de modo a responder a eventos do usuário.
