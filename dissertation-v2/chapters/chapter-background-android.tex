...
O Android Studio \cite{AndroidStudio} é o ambiente integrado de desenvolvimento (\textit{Integrated Development Environment} - IDE) oficial e gratuito, mantida pelo Google, comumente usado para o desenvolvimento Android. Juntamente com o Android Studio vem instalado o \ac{SDK}. O Android \ac{SDK} é um conjunto de ferramentas para o desenvolvimento Android, dentre as ferramentas podemos encontrar o compilador, depurador de código, emulador, bibliotecas de componentes base para a criação de aplicativos Android, dentre outras. O arquivo instalável de um aplicativo Android possui a extensão \texttt{.apk}, acrônimo para \emph{Android PacKage} \cite{AndroidFundamentals}. 
...
O Android \ac{SDK} provê diversos componentes base para o desenvolvimento Android que não estão disponíveis no desenvolvimento Java tradicional \cite{AndroidUI1:17}. Alguns componentes são utilizados no \textit{front-end}, outros para processamentos em segundo plano, outros para persistência em banco de dados local, dentre outros. Por exemplo, \textit{Activities} são usadas para a criação de telas com interface \ac{UI} com o qual usuários podem interagir \cite{AndroidActivities2016}. \textit{Services} são usados para longos processamentos em segundo plano e não possuem \ac{UI} \cite{AndroidDevServices}. \textit{BroadcastReceivers} são usados como ouvintes, registrando interesse por mensagens enviadas pelo sistema operacional Android, como bateria baixa, ou por outros componentes \cite{AndroidDevBroadcast}. \textit{AsyncTasks} são usadas para curtos processamentos em segundo plano de forma assíncrona, com o objetivo de não bloquear a \ac{UI} \textit{Thread} \cite{AndroidDevAsyncTask}. 

Cada componente do Android \ac{SDK} possui um conjunto de métodos de retorno, conhecidos por \textit{callbacks}, que podem ou devem ser sobrescritos pelo desenvolvedor e que são invocados pelo sistema operacional Android. O ciclo de vida de um componente é composto por um conjunto de \textit{callbacks} que são sempre invocados e em uma ordem específica \cite{AndroidActivities2016}. Componentes de \ac{UI} como \textit{Activities} costumam ter ciclos de vida mais extensos, e portanto mais complexos, que componentes que não possuem \ac{UI}. A Figura \ref{fig:android-lifecycles} apresenta como exemplo os ciclos de vida de \textit{Activities}, que possuem \ac{UI}, e \textit{Services}, que não possuem \ac{UI}.
...
Recursos são arquivos ``não Java'', relacionados a \ac{UI} e necessários para a criação de aplicativos Android \cite{AndroidFundamentals}. Recursos podem ser imagens, arquivos de áudio ou arquivos XML, sendo os elementos e atributos usados no XML derivados do Android \ac{SDK}. Recursos de \textit{Layout} são XMLs responsáveis pela construção da \ac{UI} como botões e caixas de textos. Recursos de \textit{String} são XMLs responsáveis pelo armazenamento dos textos usados na aplicação e possibilitam a internacionalização. Recursos de \textit{Style} são XMLs responsáveis pelo armazenamento dos estilos usados nos recursos de \textit{layout}. Recursos \textit{Drawable} são gráficos que podem ser imagens ou arquivos XML que criam animações ou efeitos de estado de botões como pressionado ou desabilitado. Apesar de ser possível implementar os recursos via código Java, é fortemente recomendado pela plataforma que isso não seja feito \cite{AndroidProvidingResources}.


\subsection{Elementos de Interface Android}

São muitos os componentes e recursos disponibilizados pelo Android \ac{SDK}. Nosso estudo objetiva analisar elementos relativos ao \textit{front-end} Android. Em termos de componentes, fizemos uma extensa revisão na documentação oficial do Android \cite{AndroidDeveloperSite2016} e chegamos em quatro, são eles: \textit{Activities}, \textit{Fragments}, \textit{Adapters} e \textit{Listeners}. 
...
\noindent \textbf{\textit{Fragments}} são como \textit{Sub-Activities}, representam parte da \ac{UI} de uma \textit{Activity} e também devem indicar seu recurso de \textit{layout} correspondente. \textit{Fragments} possuem um ciclo de vida extenso, com mais de dez \textit{callbacks}. Seu ciclo de vida está diretamente ligado ao ciclo de vida da \textit{Activity} ao qual ele está contido. A principal uso de \textit{Fragments} é para o reaproveitamento de trechos de \ac{UI} e comportamento em \textit{Activities} diferentes \cite{AndroidFragments}.
...
\noindent \textbf{\textit{Recursos de Layout}} são XMLs utilizados para o desenvolvimento da \ac{UI} dos componentes Android. O desenvolvimento é feito utilizando uma hierarquia de \textit{Views} e \textit{ViewGroups}. \textit{Views} são caixas de texto, botões, dentre outros elementos de interface. \textit{ViewGroups} são \textit{Views} especiais pois podem conter outras \textit{Views}. Cada \textit{ViewGroup} organiza suas \textit{Views} filhas de uma forma específica, horizontalmente, em tabela, posicionamento relativo, dentre outros. Esta hierarquia pode ser tão simples ou complexa quanto se precisar, mas quanto mais simples melhor o desempenho \cite{AndroidLayoutResources, AndroidLayouts}. 
...
\noindent \textbf{\textit{Recursos de String}} são XMLs utilizados para definir textos, conjunto de textos e plurais usados no aplicativo. A principais vantagens de se usar recursos de \textit{String} é o reaproveitamento dos textos em diferentes \ac{UI} e a facilidade para traduzir o aplicativo em outros idiomas \cite{AndroidStringResources}.

\noindent \textbf{\textit{Recursos de Style}} são XMLs utilizados para a definição de estilos e serem utilizados nos XMLs de \textit{layout}. A principais vantagens de utilizar recursos \textit{Styles} é separar o código de estrutura da \ac{UI} do código que define sua aparência e possibilitar a reutilização de estilos em diferentes UIs \cite{AndroidStyleResources}.

\noindent \textbf{\textit{Recursos de Drawable}} são arquivos gráficos utilizados na \ac{UI}. Estes arquivos podem ser imagens tradicionais, \texttt{.png}, \texttt{.jpg} ou \texttt{.gif}, ou XMLs gráficos \cite{AndroidDrawableResources}.
