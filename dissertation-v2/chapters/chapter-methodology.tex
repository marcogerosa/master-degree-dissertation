...
Ao final da análise constavam 389 respostas sobre boas e más práticas individualmente categorizadas em 46 categorias, onde 22, que apresentaram regularidade de ocorrência maior ou igual a três, foram consideradas para a derivação dos maus cheiros. A escolha do número 3 como número mínimo de repetições para se derivar um mau cheiro é porque evita a invariabilidade do número 1, a coincidência do número 2 e já representa alguma variação \cite{campos2001estatistica}. Das categorias com recorrência maior ou igual a três, duas foram desconsideradas por se tratarem de: um mau cheiro tradicional (\textit{Large Class}) e um aspecto da orientação a objetos (Herança). O interessante é que por acaso nos deparamos com as mesmas conclusões que algumas pesquisas anteriores, de que apps Android são fortemente afetados pelo mau cheiro \textit{Large Class} \cite{MobileSmells:13} e que é pouco ou quase nada usado herança para estruturar o \textit{design} do código \cite{Mantyla2013}. Como nosso foco não estava em avaliar a presença de maus cheiro tradicionais ou práticas de orientações a objetos em apps Android, desconsideramos esses resultados. A Tabela \ref{tab:CategoriesVSFrequency} apresenta as 22 categorias consideradas para derivação dos maus cheiros.

\begin{table}[h]
\centering
\renewcommand*{\arraystretch}{1}
\footnotesize 
\begin{tabular}{@{}p{5.5cm}cc|p{5.5cm}cc@{}}
\toprule
\textbf{Categoria} & \textbf{\#} & \textbf{Tipo} & \textbf{Categoria} & \textbf{\#} & \textbf{Tipo} \\
\hline

Lógicas em view                               & 60  &   Java      & Uso excessivo de Fragments                    & 9   &   Java      \\
Padrão de nome de recurso                     & 23  &   Recurso   & Classes de UI fazendo IO                      & 9   &   Java      \\
Recurso mágico                                & 23  &   Recurso   & Não uso de Fragment                           & 8   &   Java      \\
Views aninhados                               & 18  &   Recurso   & Longo recurso de estilo                       & 8   &   Recurso   \\
Classes de UI acopladas                       & 18  &   Java      & Recurso de string bagunçado                   & 8   &   Recurso   \\
Imagem tradicional dispensável                & 18  &   Recurso   & Atributos de estilo repetidos                 & 7   &   Recurso   \\
Comportamento Suspeito                        & 17  &   Java      & Activity Inexistente                          & 6   &   Java      \\
Mau uso do ciclo de vida                      & 14  &   Java      & Não uso de arquiteturas conhecidas            & 6   &   Java      \\
Layout longo ou repetido                      & 14  &   Recurso   & Reuso excessivo de strings                    & 6   &   Recurso   \\
Não uso do padrão View Holder                 & 13  &   Java      & Adapter complexo                              & 5   &   Java      \\
Ausência de diferentes resoluções de imagens  & 12  &   Recurso   & Listener escondido                            & 5   &   Java      \\
\toprule
\end{tabular}
\caption{Categorias derivadas com ocorrências maior ou igual a três, total de ocorrências e tipo de elemento Android afetado.}
\label{tab:CategoriesVSFrequency}
\end{table}


% A Tabela \ref{tab:CategoriesVSFrequency} apresenta as 22 categorias consideradas para derivação dos maus cheiros. A partir dessas categorias e respostas, foi extraída a primeira definição dos maus cheiros, sendo que os sintomas foram embasados nas respostas dadas. 

Cada categoria considerada deu origem a um mau cheiro de código no \textit{front-end} Android sendo 12 relacionadas a elementos Java e 10 relacionadas a recursos da aplicação.
...
Para responder a \textbf{QP2}, buscamos entender a percepção dos desenvolvedores com relação a frequência e importância dos maus cheiros derivados em seu dia a dia. Coletamos esta percepção através de um questionário online com três seções sendo a primeira para coleta de dados demográficos da mesma forma da etapa 1, a segunda para coleta da percepção de frequência dos maus cheiros no dia a dia de desenvolvimento e a terceira para coleta da percepção de importância. Foram criadas duas versões do questionário, inglês e português, e coletamos ao todo 201 respostas de desenvolvedores Android de diversos países. Com o resultado conseguimos validar que os 16 maus cheiros derivados são percebidos no dia a dia de desenvolvimento e são considerados importantes.
...
Após a validação das frases, executamos um piloto com mais dois desenvolvedores, do qual, não solicitaram alterações no questionário, considerando-o apropriado para lançamento. Essas respostas foram consideradas visto que o \textit{feedback} foi positivo e não houve necessidade de mudanças no questionário.
