...
Para responder a $_3$ validamos a percepção de desenvolvedores Android sobre códigos afetados por 7 maus cheiros, considerados mais importantes e frequentes (vide Tabela \ref{tab:selected-smells}), dos 20 propostos. Os dados foram coletados por meio de um experimento de código online (S$_3$) onde cada participante era solicitado a avaliar a qualidade de 6 códigos relativos a camada de apresentação Android. Recebemos 70 respostas. Vale ressaltar que este experimento já foi realizado em pesquisas similares, como as de Aniche et al. \cite{AnicheSmellsMVC:17, MvcSmells:16} e Fabio et al. \cite{Palomba_Do_2014}, com o mesmo objetivo de validar a percepção de desenvolvedores sobre maus cheiros de código.  
...
Foi possível coletar dados para validar estatisticamente a percepção sobre 7 dos 20 maus cheiros propostos. Para definirmos quais maus cheiros seriam avaliados no experimento, usamos o seguinte critério: maus cheiros com MO de importância e frequência maior ou igual a 3 e DP de importância e frequência inferior a 1,20. A Tabela \ref{tab:selected-smells} lista os maus cheiros avaliados no experimento.
...
Para termos maior confiabilidade sobre os resultados da validação estatística, objetivamos coletar em média 30 pontos por mau cheiro avaliado. Isso significa que cada mau cheiro teve em média 30 desenvolvedores que avaliaram um código afetado por ele. Segundo Fink (1995) \cite{Fink:95}, o tamanho da amostra se refere ao número de respondentes necessário para que os resultados obtidos sejam precisos e confiáveis, e que o aumento no tamanho da amostra diminui o erro. Moscarola (1990) \cite{Moscarola:90} resume essa ideia com ``\textit{a lei dos grandes números}'', segundo a qual ele argumenta que ``\textit{com uma amostra inferior a 30 observações se tem chances de encontrar tanto um valor errôneo ou defasado como um valor se aproximando da realidade}''. 

Deste modo, lançamos o experimento de código com 5 maus cheiros, considerados mais importantes e frequentes pelos participantes de S$_2$. Conforme o número mínimo de respostas de um dos maus cheiros fosse alcançado, removíamos ele do experimento e adicionávamos outro. Deste modo foi possível coletar respostas para avaliar a percepção de 7 maus cheiros.
